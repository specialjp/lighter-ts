...
export async function fetchMarketConfig(
  ...
  try {
    const response = await orderApi.getOrderBookDetails({ market_id: marketId });
    ...
    if (response.code === 200 && response.order_book_details.length > 0) {
      ...
      const config: MarketConfig = {
        ...
        tickSize: Math.pow(
          10,
          -(details.price_decimals - details.supported_price_decimals)
        ),
    ...
    throw new Error(
      `Failed to fetch market config for market ${marketId}: ${
...
export async function getMarketConfig(
  ...
  if (!orderApi) {
    throw new Error(
      `Market ${marketId} not configured and no OrderApi provided`
    );
  }
